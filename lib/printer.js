  ...
  switch (node.type) {
    ...
    case 'TupleExpression': {
      return group(
        concat([
          '(',
          node.expressions.length !== 0 ? softline : '',
    ...
    case 'CallExpression': {
      return group(
        concat([
          ...
          '(',
          node.arguments.length !== 0 ? softline : '',
    ...
    case 'AssignmentExpression': {
      return group(
        concat([printNode(node.left), ' = ', printExpression(node.right)])
      );
    }
...
function printUnionClause(node) {
  if (!node) {
    return '';
  }

  // eslint-disable-next-line no-use-before-define
  return concat([line, 'UNION ', printSelectStatment(node)]);
}
  ...
  switch (node.type) {
    ...
    case 'TableReference':
      return concat([
        ...
        dedent(concat([line, join(line, node.joins.map((x) => printJoin(x)))])),
      ]);
...
function printSelectStatment(node) {
  return group(
    concat([
      ...
      node.highPriority ? ' HIGH_PRIORITY' : '',
      node.straightJoin ? ' STRAIGHT_JOIN' : '',
      node.sqlSmallResult ? ' SQL_SMALL_RESULT' : '',
      node.sqlBigResult ? ' SQL_BIG_RESULT' : '',
      node.sqlBufferResult ? ' SQL_BUFFER_RESULT' : '',
      node.sqlNoCache ? ' SQL_NO_CACHE' : '',
      node.sqlCalcFoundRows ? ' SQL_CALC_FOUND_ROWS' : '',
      indent(
        concat([
          line,
          join(
