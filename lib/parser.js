...
// [] better message for trailing comma
// [] lexer
// [] not operator
// [] operators
// [] comments
...
function parseHigherThanBinaryExpression(parser) {
  ...
  switch (getChar(parser)) {
    ...
    default:
      node = parseIdentifierOpt(parser) || parseSpecialValueOpt(parser);
  }
  ...
  if (
    (node.type === 'BacktickStringLiteral' || typeof node === 'string') &&
...
function parseSubqueryOpt(parser) {
  ...
  const statement = parseStatement(parser); // eslint-disable-line no-use-before-define
  consumeChar(parser, ')');
  return statement;
}
...
function parseTableFactor(parser) {
  ...
  if (getChar(parser) === '(') {
    ...
    if (!alias) {
      throw new ParseError('Missing alias for subquery.', parser);
    }
    return {
      type: 'TableFactor',
      table: subquery,
...
function parseFromClauseOpt(parser) {
  ...
  const firstTable = parseTableReference(parser);
  const otherTables = [];
  while (consumeCharOpt(parser, ',')) {
    otherTables.push(parseTableReference(parser));
  }
...
function parseStatement(parser) {
  ...
  switch (keyword) {
    ...
    case 'INSERT': {
      ...
      let select;
      if (!values && matchKeyword(parser, 'SELECT')) {
