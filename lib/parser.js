 ...
 * @returns {string | undefined}
 */
function getWordUppercase(parser) {
  const word = /^\w+/.exec(parser.source.slice(parser.pos));
  return word === null ? undefined : word[0].toUpperCase();
}
...
function consumeKeywordOpt(parser, keyword) {
  consumeSpaces(parser);
  const word = getWordUppercase(parser);
  if (word === keyword) {
    consumeWord(parser);
    return keyword;
  }
...
function parseHigherThanBinaryExpression(parser) {
  ...
  if (
    (node.type === 'BacktickLiteral' || node.type === 'Identifier') &&
 ...
 * @param {ParserState} parser
 */
function consumeOperatorTokenOpt(parser) {
    ...
    default: {
      ...
      switch (word) {
        ...
        case 'MOD':
          ...
          return word;
        case 'IS': {
          consumeKeyword(parser, 'IS');
          const not = consumeKeywordOpt(parser, 'NOT');
          if (consumeKeywordOpt(parser, 'NULL')) {
            return not ? 'IS NOT NULL' : 'IS NULL';
          }
          if (consumeKeywordOpt(parser, 'TRUE')) {
            return not ? 'IS NOT TRUE' : 'IS TRUE';
          }
          if (consumeKeywordOpt(parser, 'FALSE')) {
            return not ? 'IS NOT FALSE' : 'IS FALSE';
          }
          throw new ParseError(parser);
        }
 ...
 */
function createPostfixExpressionNode(operator, operand) {
  return {
    type: 'UnaryExpression',
    prefix: false,
    operator,
 ...
 */
function parseOperatorOfPrecedence(parser, minPrecedence) {
  const start = parser.pos;
  consumeSpaces(parser);

  const operator = consumeOperatorTokenOpt(parser);
...
function parseExpression(parser, minPrecedence) {
  ...
  while ((operator = parseOperatorOfPrecedence(parser, minPrecedence))) {
    if (isPostfix(operator)) {
      node = createPostfixExpressionNode(operator, node);
...
function parseCreateDefinition(parser) {
  ...
  const isUniqueKey = !!(
    consumeKeywordOpt(parser, 'UNIQUE') &&
    (consumeKeywordOpt(parser, 'KEY'), true)
  );
  ...
  const comment =
    consumeKeywordOpt(parser, 'COMMENT') && parseStringLiteral(parser);
  const collate =
    consumeKeywordOpt(parser, 'COLLATE') && parseIdentifier(parser);
  const references = parseReferenceDefinitionOpt(parser);
  return {
...
function parseAssignment(parser) {
  ...
  if (!consumeTokenOpt(parser, '=')) {
    throw new ParseError(parser, 'Invalid assignment expression.')
  }
...
function parseSetClause(parser) {
  ...
  const {assignments} = parseAssignmentList(parser);
  return {
    type: 'SetClause',
    assignments,
  };
...
function parseStatement(parser) {
  ...
  switch (keyword) {
    ...
    case 'INSERT': {
      ...
      if (values) {
      } else if (matchKeyword(parser, 'SET')) {
        set = parseSetClause(parser);
