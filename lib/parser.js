...
function log(...args) {
  const {inspect} = require('util'); // eslint-disable-line global-require
...
function parseHigherThanBinaryExpression(parser) {
  let literalOrIdentifier;
  switch (getChar(parser)) {
    case '"':
    case `'`:
    case '`':
      literalOrIdentifier = parseLiteral(parser);
      break;
    default:
      literalOrIdentifier = parseIdentifier(parser);
  }
  ...
  switch (getChar(parser)) {
    case '(':
      return parseCallExpression(parser, literalOrIdentifier);
    case '.':
    default:
...
function main() {
  const text = 'create table reqwreqw (id int)';
  // const text = `0 = a + b *  c +  d ; `;
