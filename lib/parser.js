 ...
 * @param {ParserState} parser
 */
function parseStringLiteralOptLEGACYTODO(parser) {
  consumeSpaces(parser);
  let value;
  let type;
  switch (getChar(parser)) {
...
function parserIdentifierOrBacktickLiteral(parser) {
  ...
  if (getChar(parser) === '`') {
    return parseStringLiteralOptLEGACYTODO(parser);
  }
...
function parseHigherThanBinaryExpression(parser) {
  ...
  switch (getChar(parser)) {
    ...
    case `'`:
    case '`':
      node = parseStringLiteralOptLEGACYTODO(parser);
      break;
    case '*':
      advanceOne(parser);
      return {
...
function parseSelectExpression(parser) {
  ...
  if (consumeKeywordOpt(parser, 'AS')) {
    const alias =
      parseStringLiteralOptLEGACYTODO(parser) || parseIdentifier(parser);
  ...
  const alias =
    parseStringLiteralOptLEGACYTODO(parser) || parseIdentifierOpt(parser);
...
function parseCreateDefinition(parser) {
  ...
  if (consumeKeywordOpt(parser, 'DEFAULT')) {
    ...
    if (!defaultValue && consumeTokenOpt(parser, '(')) {
      defaultValue = parseExpression(parser, 0);
      consumeToken(parser, ')');
    }
    if (!defaultValue) {
      throw new ParseError(parser);
...
function parseTableOption(parser) {
  ...
  if (consumeKeywordOpt(parser, 'COMMENT')) {
    consumeTokenOpt(parser, '=');
    const value = parseStringLiteralOptLEGACYTODO(parser);
    if (!value) {
      throw new ParseError(parser);
    }
    return {
